...
module mem1(
    ...
    input  wire [3:0]  mem_wd_reg_i,  // 内存写掩码
    input  wire [31:0] mem_wd_addr_i, // 内存写物理地址
    input  wire [31:0] mem_wd_data_i, // 要写入内存的数据 
    input  wire        is_load_i,     // 是否是 load 指令
    
    output wire [4:0]  rd_addr_o,      
    output wire  [31:0] rd_data_o,     // 注意：这里是 reg，因为在 always 块中赋值 
    ...
    assign rd_data_o     = rd_data_i;       // PASS THROUGH ALU RESULT
    assign mem_wd_reg_o  = mem_wd_reg_i;
    assign mem_wd_addr_o = mem_wd_addr_i;
    assign mem_wd_data_o = mem_wd_data_i;
